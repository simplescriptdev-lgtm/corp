...
$in_total = (float)$pdo->query("SELECT COALESCE(SUM(amount),0) s FROM capital_inflows WHERE category='operational'")->fetch()['s'];
$out_total= (float)$pdo->query("SELECT COALESCE(SUM(amount),0) s FROM operational_withdrawals")->fetch()['s'];
$remain = $in_total - $out_total;
$hist = $pdo->query("SELECT * FROM operational_withdrawals ORDER BY created_at DESC,id DESC")->fetchAll();
?>
<div class="card2">
  <div class="title">Операційний капітал</div>
  <table>
    <tbody>
      <tr><td>Загальне надходження операційної діяльності</td><td class="text-end"><strong><?= mUAH($in_total) ?></strong></td></tr>
    </tbody>
  </table>
</div>
<div class="card2">
  ...
  <div class="actions"><button class="btn primary" onclick="document.getElementById('opW').showModal()">Вивести капітал</button></div>
</div>
<div class="card2">
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="text-end">
          <button class="btn outline" onclick="document.getElementById('opE<?= $id ?>').showModal()">Редагувати</button>
          <form method="post" action="insurance.php" style="display:inline" onsubmit="return confirm('Видалити запис?')">
            <input type="hidden" name="action" value="op_withdraw_delete"><input type="hidden" name="id" value="<?= $id ?>">
            <button class="btn danger">Видалити</button>
          </form>
        </td>
      </tr>
      <dialog id="opE<?= $id ?>"><form method="post" action="insurance.php" class="card">
        <h3>Редагувати</h3>
        <input type="hidden" name="action" value="op_withdraw_update"><input type="hidden" name="id" value="<?= $id ?>">
...
<dialog id="opW"><form method="post" action="insurance.php" class="card">
  <h3>Вивести капітал</h3>
  <input type="hidden" name="action" value="op_withdraw_create">
  <label>Сума (₴)</label><input type="number" step="0.01" name="amount" required>
  <label>Нотатка</label><input name="note">
  <div class="actions"><button type="button" class="btn" onclick="document.getElementById('opW').close()">Скасувати</button><button class="btn primary">Зберегти</button></div>
</form></dialog>
